<div>
                ...
                for (const [dateKey, tasks] of Object.entries(source)) {
                    ...
                    if (this.prioritySortOrder !== 'none') {
                        sortedTasks = sortedTasks.sort((a, b) => {
                            if (this.prioritySortOrder === 'high-low') {
                                return priorityIndexes[b.priority] - priorityIndexes[a.priority];
                            } else if (this.prioritySortOrder === 'low-high') {
    ...
    >
        ...
        <div class="flex gap-2 mb-3 items-center">
            ...
            <!-- PRIORITY SORT -->
            <button
                @click="prioritySortOrder = prioritySortOrder === 'none' ? 'low-high' : prioritySortOrder === 'low-high' ? 'high-low' : 'none'"
                :class="prioritySortOrder === 'none' ? 'border-gray-600 text-gray-700 font-bold' : prioritySortOrder === 'low-high' ? 'border-gray-500 text-gray-600 font-bold' : 'border-gray-600 text-gray-700 font-bold'"
                class="px-1.5 py-0.5 rounded-md shadow border hover:bg-gray-100 transition text-xs min-w-[120px] whitespace-nowrap"
            >
                Priority:
                <span x-text="prioritySortOrder === 'none' ? 'None' : prioritySortOrder === 'low-high' ? 'Low → High' : 'High → Low'"></span>
            </button>
            <!-- STATUS SORT -->
            <button
                @click="statusSortOrder = statusSortOrder === 'none' ? 'todo-done' : statusSortOrder === 'todo-done' ? 'done-todo' : 'none'"
                :class="statusSortOrder === 'none' ? 'border-gray-600 text-gray-700 font-bold' : statusSortOrder === 'todo-done' ? 'border-gray-500 text-gray-600 font-bold' : 'border-gray-600 text-gray-700 font-bold'"
                class="px-1.5 py-0.5 rounded-md shadow border hover:bg-gray-100 transition text-xs min-w-[120px] whitespace-nowrap"
            >
                Status:
                <span x-text="statusSortOrder === 'none' ? 'None' : statusSortOrder === 'todo-done' ? 'To-Do → Done' : 'Done → To-Do'"></span>
            </button>
        </div>
